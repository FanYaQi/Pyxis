...
def load_flare_data(filepath, country, year):
    ...
    flare_aggregated = (
        flare_filtered.groupby(["id"], as_index=False)
        .agg({"lat": "first", "lon": "first", "t_mean": "mean", "BCM": "sum"})
        .rename(
            columns={
                "lat": "Latitude",
                "lon": "Longitude",
    ...
    flare_aggregated["geometry"] = flare_aggregated.apply(
...
def match_flares_to_fields(flare_gdf, field_gdf):
    ...
    initial_match_volume = flare_gdf[flare_gdf["id"].isin(unique_matched_ids)][
        "BCM_2021"
    ].sum()
    total_flare_volume = flare_gdf["BCM_2021"].sum()
    initial_match_rate = (
        initial_match_volume / total_flare_volume if total_flare_volume != 0 else 0
    )
    ...
    extended_matches = gpd.sjoin(
        flare_gdf[~flare_gdf["id"].isin(unique_matched_ids)],
        buffered_fields,
        how="left",
        predicate="within",
    )
    ...
    if not extended_matches.empty:
        ...
        if not extended_overlaps.empty:
            ...
            ] / extended_overlaps.groupby("id")["Oil production volume"].transform(
                "sum"
            )
    ...
    if not unmatched_flares.empty:
        all_matches.loc[
            unmatched_flares.index, "weight"
        ] = 1  # Assign weight of 1 for unmatched flares to retain original volume
    ...
    all_matches["weight"] = all_matches.apply(
        lambda row: row["weight"] / row["weight_sum"]
        if row["weight_sum"] > 0
        else row["weight"],
        axis=1,
    )
    ...
    combined_matches = (
        all_matches.groupby("index_right")
        .agg({"weighted_volume": "sum", "Oil production volume": "first"})
        .reset_index()
    )
    combined_matches["Flaring-to-oil ratio"] = (
        combined_matches["weighted_volume"]
        * BCM2scf
        / (combined_matches["Oil production volume"] * 365)
    )

    # Update the existing Flaring-to-oil ratio column in the field data
    field_gdf["Flaring-to-oil ratio"] = field_gdf.apply(
        lambda row: combined_matches[combined_matches["index_right"] == row.name][
            "Flaring-to-oil ratio"
        ].sum()
        if row.name in combined_matches["index_right"].values
        else row["Flaring-to-oil ratio"],
        axis=1,
    )
    ...
    final_match_volume = (
        (
            field_gdf["Oil production volume"].apply(
                lambda x: x if pd.notna(x) and x > 0 else 1
            )
            * field_gdf["Flaring-to-oil ratio"]
        ).sum()
        * 365
        / BCM2scf
    )
    final_match_rate = (
        final_match_volume / total_flare_volume if total_flare_volume != 0 else 0
    )
...
def main():
    ...
    (
        ...
        initial_match_rate,
        final_match_rate,
        unmatched_flare_volume,
    ) = match_flares_to_fields(flare_gdf, field_gdf)
    ...
    final_merged_data.drop(
        columns=[
            ...
            "Source ID used",
            "geometry",
            "Detailed in use field",
        ]
    ).to_csv(f"{DATA_PATH}/br_geodata/merged_pyxis_field_info_with_flare_withwm.csv")
...
def main_wowm():
    ...
    ).to_csv(f"{DATA_PATH}/br_geodata/merged_pyxis_field_info_with_flare_wowm.csv")
