...
def load_and_filter_fields(filepath):
    ...
    return oil_fields[oil_fields['Oil production volume'].notna() & (oil_fields['Oil production volume'] > 0)]
...
def clean_data(input_data):
    ...
    input_data.loc[input_data['Number of producing wells'] < 1, 'Number of producing wells'] = None
    input_data.loc[input_data['Number of water injecting wells'] < 1, 'Number of water injecting wells'] = None
    ...
    input_data.loc[(input_data[api_column] < 3) | (input_data[api_column] > 88), api_column] = None
            ...
            for col in ['A', 'B', 'C']:
                for cell in ws[col]:
                    if cell.value == column_name:
                        ws.cell(row=cell.row, column=column_offset, value=row[column_name])
                        found = True
                        break
                if found:
                    break
...
def main():
    # File paths
    data_filepath = f'{DATA_PATH}/br_geodata/merged_pyxis_field_info_with_flare_withwm.csv'
...
def main_wowm():
    # File paths
    data_filepath = f'{DATA_PATH}/br_geodata/merged_pyxis_field_info_with_flare_wowm.csv'
    excel_template_path = f'{DATA_PATH}/OPGEE_model/OPGEE_3.0c_BETA.xlsm'
