...
# paper_op_table['Field age'] = (['Field age'], process_age_to_year)
  ...
  data_files=[]
  for paper in papers:
      data = pd.read_excel(f'{DATA_PATH}/br_geodata/paper/'+str(paper)+'_data.xlsx')
      # data = pd.read_excel('./data/br_geodata/spe/'+str(paper)+'_data.xlsx')
      update_paper_data(data.set_index('Field ID').T,name=paper,n_explain=papers[paper]['n_explain'],type=papers[paper]['type'],
