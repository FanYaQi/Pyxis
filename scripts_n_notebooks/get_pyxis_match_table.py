    ...
    if index1 is not None and index2 is not None:
        try:
            ...
            if grid_distance < 50:
                ...
                geo_score = 100 * np.exp(
                    -0.5 * np.power(grid_distance * 0.1, 2)
...
def filter_pyxis_match(df, government_source_id, other_sources):
    ...
    id_map = {
        old_id: new_id
        for new_id, old_id in enumerate(sorted(filtered_df["Pyxis ID"].unique()))
    }
...
def main():
    ...
    filtered_df = filter_pyxis_match(
        pyxis_match_table, "anp2024", ["wm2022", "zhan2021", "gogi2023"]
    )

    # Save the Pyxis Match Table
    filtered_df.to_csv(
        f"{DATA_PATH}/br_geodata/pyxis_match_table_filtered_withwm.csv", index=False
    )
...
def main_validation():
    ...
    metadata_path = f"{DATA_PATH}/br_geodata/data_standardization/source_metadata.csv"
    data_files = [
        f"{DATA_PATH}/br_geodata/data_standardization/zhan_10.csv",
    ...
    # Save the Pyxis Match Table
    filtered_df.to_csv(
        f"{DATA_PATH}/br_geodata/pyxis_middle_version/pyxis_match_table_validation_filtered_pipeline.csv",
        index=False,
    )
...
def main_wowm():
    ...
    metadata = load_metadata(metadata_path)
    metadata = metadata[
        metadata["Source ID"] != "wm2022"
    ]  # Exclude wm source from metadata
    ...
    # Filter and reorganize the Pyxis Match Table
    filtered_df = filter_pyxis_match(
        pyxis_match_table, "anp2024", ["zhan2021", "gogi2023"]
    )

    # Save the Pyxis Match Table
    filtered_df.to_csv(
        f"{DATA_PATH}/br_geodata/pyxis_match_table_filtered_wowm.csv", index=False
    )
...
def main_iter():
    ...
    metadata = load_metadata(metadata_path)
    sources = {
        name: load_source_data(Path(data_file))
    ...
    sorted_metadata = sort_sources_by_score(metadata)
    sorted_sources = [
        next(src for key, src in sources.items() if src["Source ID"].iloc[0] == id)
        for id in sorted_metadata["Source ID"]
    ]

    # Define versions with increasing number of sources using sorted sources
    versions = {f"v{i+1}": sorted_sources[: i + 1] for i in range(len(sorted_sources))}
    ...
    for version, selected_sources in versions.items():
        ...
        pyxis_match_table.to_csv(
            f"{DATA_PATH}/br_geodata/pyxis_middle_version/pyxis_match_table_{version}.csv",
            index=False,
        )
