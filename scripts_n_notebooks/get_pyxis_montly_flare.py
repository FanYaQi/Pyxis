...
def load_flare_data(filepath, country, year):
    ...
    flare_filtered['geometry'] = flare_filtered.apply(lambda row: Point(row['lon'], row['lat']), axis=1)
...
def match_flares_to_fields(flare_gdf, field_gdf):
    ...
    if not matches_within_fields.empty:
        overlaps = matches_within_fields.groupby(['id', 'month']).filter(lambda x: len(x) > 1)
        if not overlaps.empty:
            overlaps['weight'] = overlaps['Oil production volume'] / overlaps.groupby(['id', 'month'])['Oil production volume'].transform('sum')
    ...
    extended_matches = gpd.sjoin(flare_gdf[~flare_gdf['id'].isin(matches_within_fields['id'])], buffered_fields, how='left', predicate='within')
    ...
    if not extended_matches.empty:
        extended_overlaps = extended_matches.groupby(['id', 'month']).filter(lambda x: len(x) > 1)
        if not extended_overlaps.empty:
            extended_overlaps['weight'] = extended_overlaps['Oil production volume'] / extended_overlaps.groupby(['id', 'month'])['Oil production volume'].transform('sum')
    ...
    weight_sums = all_matches.groupby(['id', 'month'])['weight'].sum().reset_index().rename(columns={'weight': 'weight_sum'})
    ...
    all_matches['month'] = all_matches['month'].astype(str).str.extract(r'(\d{1,2})').astype(int)

    # Group by field, month, and flare ID to calculate monthly flare volume considering weighted volumes
    monthly_flare_match = all_matches.groupby(['index_right', 'month']).agg({
        'weighted_volume': 'sum'
    }).reset_index()

    # Create a DataFrame with all fields and all 12 months
    all_fields_months = pd.MultiIndex.from_product([field_gdf.index, range(1, 13)], names=['index_right', 'month']).to_frame(index=False)
    monthly_flare_summary = pd.merge(all_fields_months, monthly_flare_match, on=['index_right', 'month'], how='left').fillna(0)

    # Add field names to the DataFrame
    result = monthly_flare_summary.merge(field_gdf[['Field name']], left_on='index_right', right_index=True, how='left')
...
def main():
    ...
    monthly_flare_data.to_csv(f'{DATA_PATH}/br_geodata/flare_2021/pyxis_field_monthly_flare_match_2021.csv', index=False)
    print("Monthly flare data matched to Pyxis fields and saved to 'pyxis_field_monthly_flare_match_2021.csv'")
