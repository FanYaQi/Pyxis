...
def parse_production_methods(row, production_method_col):
    ...
    production_methods = ",".join(
        (row[col].replace(" ", "").lower() if row[col] else "")
        for col in production_method_col
        if col in row
    )
...
op_table = {
    "zhan": {
        ...
        "Function unit": (
            ["Product_Ty"],
            process_function_unit,
        ),  # zhan's data is empty for this column
    ...
    "wm": {
        ...
        "Production method": (
            ["field_dr_1", "field_dr_2", "field_dr_3"],
...
class DataSource(ABC):
    ...
    def process(self):
        ...
        for _, row in self.data.iterrows():
            processed_row = {
                col: None for col in self.target_schema
            ...
            if "geometry" in row and not pd.isnull(row.geometry):
                processed_row["Centroid H3 Index"] = h3.geo_to_h3(
                    row.geometry.centroid.y, row.geometry.centroid.x, self.h3_res
                )
            ...
            for new_col_name, (input_cols, func) in self.config.items():
                if new_col_name == "Production method":
                    ...
                    processed_row[new_col_name] = (
                        func(*inputs) if len(inputs) == len(input_cols) else None
                    )  # Only proceed if all columns exist
            field_name = processed_row["Field name"]
            processed_row["Name"] = (
                unidecode(field_name).lower() if field_name else None
            )
    ...
    def data_score(
        self, score_cols, score_weight=[0.5, 0.3, 0.2]
    ):  # calculation of the data quality score
        ...
        self.data_score_avg = sum(
            [score_weight[i] * score_cols[i] for i in range(len(score_weight))]
        )
...
def main():
    ...
    anp = DataSource(
        data=anp_data,
        name="anp",
        n_explain="National Agency for Petroleum, Natural Gas and Biofuels",
        type="government",
        time="2024",
    ...
    gogi = DataSource(
        data=gogi_data,
        name="gogi",
        n_explain="National Energy Technology Laboratory",
        type="national lab",
        time="2023",
