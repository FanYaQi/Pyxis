...
# Load data
anp_well_data = pd.read_excel('./db/data/br_geodata/anp/raw/ANP_well_data_translated.xlsx')
field_registration_data = pd.read_excel('./db/data/br_geodata/anp/raw/field registration data_translated.xlsx')
field_movement_data = pd.read_excel('./db/data/br_geodata/anp/raw/translated_data_for_field_movement_2021.xlsx')
campos_gishub = gpd.read_file('./db/data/br_geodata/anp/raw/ANP field shape/campos_gishub_db.shp')

# 1. Process ANP Well Data
def process_anp_well_data(df):
    df = df.rename(columns={'Field (Well)': 'Field name', 'Final drilling depth (m)': 'Final drilling depth'})
...
def process_field_registration_data(df):
    df = df.rename(columns={'Field': 'Field name', 'API Petroleum Grade': 'API', 'Start of Production': 'Start of Production','Location':'Offshore'})
...
def process_field_movement_data(df):
    ...
    total_days = df.groupby('Field name')['days_in_month'].sum().reset_index().rename(columns={'days_in_month': 'total_days'})
...
def process_campos_gishub(df):
    df = df.rename(columns={'name': 'Field name', 'fluido_pri': 'Function unit', 'etapa': 'Field status'})
...
combined_data.to_file('./db/data/br_geodata/anp/raw/combined_anp_data.shp', encoding='utf-8')

print("Data cleaning and combination complete. The final shapefile is saved as 'combined_anp_data.shp'")